...
image_path = '/data/liukai/likeyao/images/BAID/'
reddit_art_path = '/data/liukai/likeyao/images/ArtFromReddit/'
reddit_photocritique_path = '/data/liukai/likeyao/images/selected_image_3000/train'
...
class BBDataset(Dataset):
    def __init__(self, file_dir='dataset', type='train', test=False):
        ...
        for i in tqdm(range(len(pic_paths))):
            pic_path = os.path.join(image_path+'images', pic_paths[i])
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print('image at'+pic_path+' read ERROR!!!!!!!!!!')
...
class RedditArtDataset(Dataset):
    def __init__(self):
        self.test_transformer = transforms.Compose(
            [
                transforms.ToPILImage(),
        ...
        pic_paths = DATA['image'].values.tolist()
        for i in tqdm(range(len(pic_paths))):
            pic_path = os.path.join(reddit_art_path+'images', pic_paths[i])
            label = float(labels[i] / 10)
            self.pic_paths.append(pic_path)
            self.labels.append(label)
...
class RedditPhotocritiqueDataset(Dataset):
    def __init__(self):
        self.test_transformer = transforms.Compose(
            [
                transforms.ToPILImage(),
        ...
        DATA  = pd.read_csv('/home/liukai/likeyao/AestheticScore/DataSet/3000/image_score_512_train.csv')

        labels = DATA['score'].values.tolist()
        pic_paths = DATA['image'].values.tolist()
        for i in tqdm(range(len(pic_paths))):
            pic_path = os.path.join(reddit_photocritique_path, pic_paths[i])
            label = float(labels[i] / 10)
            self.pic_paths.append(pic_path)
            self.labels.append(label)
